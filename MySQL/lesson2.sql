...
select *
from client
where right(LastName, 3) = 'ova';

select *
from client
where substr(LastName, length(LastName) - 1) = 'ov'
   or substr(LastName, length(LastName) - 2) = 'ova';
...
select *
from client
where substr(LastName, length(LastName) - 2) = 'ova';
...
select c.*
from client c
where c.idClient in (select a.Client_idClient
                     from application a
                     where a.CreditState = 'Not returned'
                       and a.Sum > 5000);

-- 9. Порахувати кількість клієнтів усіх відділень та лише львівських відділень.
select d.idDepartment, d.DepartmentCity, count(*)
from client c
         join department d on d.idDepartment = c.Department_idDepartment
...
set c.City = 'Kyiv'
where c.City <> 'Kyiv'
  and c.Department_idDepartment in (select d.idDepartment
                                    from department d
                                    where d.DepartmentCity = 'Kyiv');
...
select c.*
from client c
where c.idClient in (select a.Client_idClient from application a where a.CreditState = 'Returned' and a.Sum > 5000);
...
select c.*
from client c
         join (select c.*, count(*) credit_count
               from client c
                        join application a on c.idClient = a.Client_idClient
               group by c.idClient
               order by credit_count desc
               limit 1) c0 on c0.City = c.City and c0.idClient <> c.idClient;

-- 27. Місто клієнта з найбільшою кількістю кредитів
select c.City
from (select c.City, count(*) credit_count
      from client c
               join application a on c.idClient = a.Client_idClient
      group by c.idClient
      order by credit_count desc
